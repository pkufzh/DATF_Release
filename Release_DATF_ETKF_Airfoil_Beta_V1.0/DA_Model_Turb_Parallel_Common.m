...
S_Va_turb_Det_base_lower=[80,80,80,80];  % 初始扰动变量下区间
S_Va_turb_Det_base_upper=[80,80,80,80];  % 初始扰动变量上区间
S_Gap=5; % 定义最宽区间状态扰动步长（百分比，即数值 5 代表具有最大扰动区间的变量以 5% 步长变化）
...
% S_Va_turb_Det_base_ratio=zeros(1,length(S_Va_turb_Std));
% for i=1:length(S_Va_turb_Std)
%     S_Va_turb_Det_base_ratio(i)=(S_Va_turb_Det_base_range(i))/(N-1);
% end
% N=max(S_Va_turb_Det_base_lower,S_Va_turb_Det_base_upper)+1;
...
S_Va_init_Combined=State_Ensem_Gen_Parellel_Common_Assist(S_Va_init,N,MStepNS_cov,targetPath);
